...
> **建议先备份** ，操作正确不会影响数据，但误操作有很大概率导致 **配置丢失** 。更新前把整个文件夹复制一份，或者至少把 `config.json` 和 `WebView2Profile` 单独拷出来。

CI构建产物不包括 `config.json`（设置文件）和 `WebView2Profile`（Cookie / 登录态 / 缓存）等，**直接解压覆盖** 不会影响设置和Cookie。但构建脚本 `publish.ps1` 会**清空** `artifacts/portable`。

### 关于本软件的定位

本软件**相当于给你的 dsh 换了个浏览器外壳**：它跑的还是 `dsh web`，只是把页面放进了 WebView2 而不是你日常用的浏览器。

这意味着 **`WebView2Profile` 是一个全新的、独立的浏览器配置目录**，受此影响：

- **部分 dsh 插件的数据可能无法同步。** 部分插件使用浏览器的 `localStorage` / `IndexedDB` / Cookie 存储配置。

不会影响你原有的浏览器。

### 主窗口

- **拖动层**：页面顶部有一条透明拖动区域（默认完全不可见但仍可拖动，会阻挡页面交互；右键菜单「拖动层」可开关）
...
- **开发提示**：DSH工作区不要包含程序目录，详见「DSH工作区不要包含程序目录」节
...
| 在软件内运行 dsh 时，工作区把程序目录包在里面，终端报 temp 相关错误 | 见下方「DSH工作区不要包含程序目录」一节                                                           |
